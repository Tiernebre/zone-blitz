...
import {
  ...
  type QualityTier as ContractQualityTier,
...
// Maps the 5-tier generation band onto the 3-tier contract-structure
// tiering (star/starter/depth) that drives length/bonus/guarantee
// priors in contract-structure-bands.ts.
function bandToContractQualityTier(band: QualityTier): ContractQualityTier {
  if (band === "elite") return "star";
  if (band === "strong" || band === "average") return "starter";
  return "depth";
}
...
function rollContract(
  ...
  args: {
    ...
    bucket: NeutralBucket;
    quality: number;
    qualityTier: ContractQualityTier;
    age: number;
    signedYear: number;
    ...
    generateContracts(
      ...
      for (const [teamId, teamPlayers] of byTeam) {
        teamPlayers.forEach((p, idx) => {
          ...
          const slotTier = qualityTierForBucketSlot(
            rng,
            bucket,
            indexInBucket,
            bucketCount,
          );
          const quality = rollQuality(rng, bucket, slotTier);
          ...
          enriched.push({
            ...
            bucket,
            quality,
            qualityTier: bandToContractQualityTier(slotTier),
            age,
          });
        });
      }
