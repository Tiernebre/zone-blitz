...
import app.zoneblitz.league.hiring.hire.StaffContractFactory;
import app.zoneblitz.league.hiring.hire.StaffContractRepository;
...
public class PreferenceScoringOfferResolver implements OfferResolver {
  ...
  private final List<CpuTeamStrategy> cpuStrategies;
  ...
  private Optional<CpuHiringStrategy> cpuStrategyFor(LeaguePhase phase) {
    return cpuStrategies.stream()
        .filter(s -> s.phase() == phase)
        .filter(CpuHiringStrategy.class::isInstance)
        .map(CpuHiringStrategy.class::cast)
        .findFirst();
  }
