...
[League Setup](./league-setup.md) for the full vision. Simulation- specific
implications:
...
- **Compressed Year 1 regular season.** An 8-team initial league plays a shorter
  regular-season schedule (~10–12 games) than the standard 17-game season. The
  sim should treat schedule length as a league parameter rather than a
  hard-coded 17.
- **Attributes are normalized per league.** The sim consumes player attributes
  on their league-local scale — a 90+ rating in a new league is the league's top
  tier, full stop, without reference to any external scale. See
  [Player Attributes](./player-attributes.md).
- **Year-1 talent has wider variance.** The initial player pool contains
